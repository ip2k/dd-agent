...
* [FEATURE] Elasticsearch: Add tags to events. See [#1444][] [TEST]
...
* [FEATURE] JMXFetch: Add service check count to JMX checks statuses. See [#1559][] [TEST]
* [FEATURE] Mesosphere: New checks for masters -reporting metrics from leaders- and slaves -reporting metrics from the selected tasks-. See [#1535][] [TEST]
...
* [FEATURE] System: Add system swap metrics. See [#1549][] [TEST]

* [IMPROVEMENT] Only start the Collector and Dogstatsd when needed. See [#1569][]
* [IMPROVEMENT] Use internal `/run` for temporary pid and pickle files. See [#1569][] [TEST]
* [IMPROVEMENT] Disk: New check based on `psutil` replaces the old system check. See [#1596][] [TEST]
* [IMPROVEMENT] NTP: Use Datadog NTP pool. See [#1618][]
* [IMPROVEMENT] Process: Cache AccessDenied failures and PID list. See [#1595][]
* [IMPROVEMENT] SQL Server: Set a timeout for each SQL command, default to 30s. See [#1446][] [TEST]
...
* [BUGFIX] Docker: Fix intermittent failures (bad support) when a container has no name. See [#1565][] [TEST]
* [BUGFIX] HTTP Check: Fix status type errors in service check. See [#1644][]
* [BUGFIX] JMXFetch: Clean previous JMX status python file at start. See [#1655][] [TEST]
...
* [BUGFIX] SQL Server: Fix collector freezes when connection is failing. See [#1640][] [TEST]
* [BUGFIX] SQL Server: Properly close cursor, avoid leaks. See [#1631][] [TEST]
* [BUGFIX] SQL Server: Send fractional metrics as floats. See [#1616][] [TEST]
* [BUGFIX] Varnish: Allow tags in varnish 3 XML style parsing. See [#1645][]
* [BUGFIX] WMI: Cast tag values as strings. See [#1630][] [TEST]

### Pending changes
* [FEATURE] Environment variable substitution support in configuration files. See [#1343][]
* [FEATURE] Service metadata support. See [#1611][]
* [FEATURE] MySQL: More metrics. See [#1436][] [#1284][] [#1274][] [#1269][]
